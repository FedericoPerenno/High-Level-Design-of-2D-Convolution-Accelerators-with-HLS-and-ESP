...
static void conv2d_sw(token_t *input, token_t *output)
{
...
static void init_buf (token_t *in, token_t * gold)
{
	...
	// initialize golden input
    float out_local_gold[batch*OUT_SIZE];
	for (i = 0; i < batch; i++)
    {		
...
#else
	print_uart("Found "); print_uart_int(ndev); print_uart(" devices: sld,conv2d_cxx\n");
#endif

	if (ndev == 0) {
#ifndef __riscv
		printf("conv2d_cxx not found\n");
#else
...
#else
	print_uart("  gold buffer base-address = "); print_uart_addr((uintptr_t) gold); print_uart("\n");
#endif
