...
directive set -MEM_MAP_THRESHOLD 325
...
#directive set -CLOCKS {clk {-CLOCK_PERIOD 50.0 -CLOCK_EDGE rising -CLOCK_UNCERTAINTY 0.0 -RESET_SYNC_NAME rst -RESET_ASYNC_NAME arst_n -RESET_KIND sync -RESET_SYNC_ACTIVE high -RESET_ASYNC_ACTIVE low -ENABLE_ACTIVE high}}
...
# Add source files.
solution file add ../src/$uarch/conv2dlb.cpp -type C++
solution file add ../inc/conv2dlb.hpp -type C++
solution file add ../tb/main.cpp -type C++ -exclude true

solution file set ../src/$uarch/conv2dlb.cpp -args -DDMA_WIDTH=$DMA_WIDTH
solution file set ../inc/conv2dlb.hpp -args -DDMA_WIDTH=$DMA_WIDTH
...
if {$opt(hsynth)} {
    ...
    if {$opt(hier)} {
        ...
        directive set /$ACCELERATOR/load/core/LOAD_BATCH_LOOP -PIPELINE_INIT_INTERVAL 1
        directive set /$ACCELERATOR/load/core/LOAD_BATCH_LOOP -PIPELINE_STALL_MODE flush
...
if {$opt(hier)} {
	project report -filename "/home/perenno/esp/accelerators/catapult_hls/${ACCELERATOR}/hw/hls/Results/report_hier.txt"  -area 1 -bom 1 -clock 1 -loops 1 -memories 1 -registers 1 -summary 1 -timing 1

} else {
	project report -filename "/home/perenno/esp/accelerators/catapult_hls/${ACCELERATOR}/hw/hls/Results/report_basic.txt"  -area 1 -bom 1 -clock 1 -loops 1 -memories 1 -registers 1 -summary 1 -timing 1
}
