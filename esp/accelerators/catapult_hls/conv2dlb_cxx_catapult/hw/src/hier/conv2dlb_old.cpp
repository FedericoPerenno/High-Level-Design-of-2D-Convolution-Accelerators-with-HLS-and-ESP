					...
					// Force serialization between DMA control and DATA data transfer
		   			if (dma_read_ctrl_done) { 
				   		for (uint8_t row = 0; row < N_W_IN_MAX; row++)
						{
							...
							if (row == kern - 1)
							{
								ESP_REPORT_INFO(VON, "Buffer triple is being written");
						 		buf_linear.write(buf_tmp_lin);
							}
							else if (row > kern - 1)
							{
								ESP_REPORT_INFO(VON, "Buffer is being written");
								buf_linear.write(buf_tmp_lin);
							}  
...
void compute(
    ...
    const uint8_t pad = ((stride * (n_w - 1) - n_w + kern) / 2) * same;
	const uint8_t n_w_out = (n_w + 2 * pad - kern) + 1;
	const uint8_t n_h_out = (n_h + 2 * pad - kern) + 1;
					...
					for (uint8_t j = 0; j < N_H_OUT_MAX; j++)
					{
						...
						for (uint8_t m = 0; m < KERN_MAX; m++)
						{
							...
							{								
								...
								//ESP_REPORT_INFO(VON, "buf_lin[%u][%u] * plm_f[%u] -->> %f x %f = %f", ESP_TO_UINT32(x_tmp), ESP_TO_UINT32(y), ESP_TO_UINT32(index_f), buf_tmp_lin.data[x_tmp][y].to_double(), plm_tmp_f.data[index_f].to_double(), acc.to_double());
						...
						if (k == n_c - 1){
							...
							plm_tmp_out.data[index_out] = buf_tmp_acc.data[i][j];
...
STORE_BATCH_LOOP:
    for (uint8_t b = 0; b < BATCH_MAX; b++) {
        ...
        uint16_t dma_write_data_index = (n_w * n_h * n_c + kern * kern * n_c * filt) * batch + dma_write_data_length * b;
...
void CCS_BLOCK(conv2dlb_cxx_catapult)(
    ...
    compute(plm_conf_compute, buf_linear, plm_kernel, plm_outputs, compute_done);
